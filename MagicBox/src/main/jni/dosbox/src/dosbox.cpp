...
void DOSBOX_UnlockSpeed( bool pressed ) {
	...
	static Bit32s savedCycleMax = 0;//locnet, 2011-06-02
	if (pressed) {
		...
		//locnet, 2011-06-02, set cycle to 1000 when unlock speed
		else if (savedCycleMax == 0) {
			savedCycleMax = CPU_CycleMax;
			CPU_CycleMax=1000;
		}
	} else {
		...
		//locnet, 2011-06-02, restore cycle after unlock speed
		else if (savedCycleMax != 0) {
			CPU_CycleMax=savedCycleMax;
			savedCycleMax = 0;
		}
	}
}

void SaveGameState(const std::string &path)
{
	if (path.empty())
		return;
...
void LoadGameState(const std::string &path)
{
	if (path.empty())
		return;
...
void DOSBOX_Init(void) {
	...
	char	buf[10]=""; 
	if (loadf->cycles) 
		sprintf(buf, "%d", loadf->cycles);
	
	Pstring = Pmulti_remain->GetSection()->Add_string("parameters",Property::Changeable::Always,buf);
#else
...
#endif
{
	...
	const char *devices[] = { "default", "win32", "alsa", "oss", "coreaudio", "coremidi",
#if C_FLUIDSYNTH
							  "synth",
#endif
							  "mt32", "none", 0};
...
#if C_FLUIDSYNTH
	Pint = secprop->Add_int("synth.sample-rate",Property::Changeable::WhenIdle,48000);
	Pint->Set_values(oplrates);

	Pint = secprop->Add_int("synth.gain",Property::Changeable::Always,6);
	Pint->SetMinMax(0,100);
#endif
