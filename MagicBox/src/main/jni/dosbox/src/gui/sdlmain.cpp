...
void GFX_SetTitle(Bit32s cycles,Bits frameskip,bool paused){/*android apps have no window title*/}
...
Bitu GFX_SetSize(Bitu width,Bitu height,Bitu flags,double scalex,double scaley,GFX_CallBack_t callback) {
	...
	Android_SetVideoMode(width,height,bpp);


	//loadf->scale_width = width;
	//loadf->scale_height = height;
...
void GFX_EndUpdate( const Bit16u *changedLines ) 
{
	...
	Bitu y=0, index=0, startLine=0, endLine=0;
	if (changedLines) {
...
/*
	if (changedLines) {
		...
		for (int i = 0; i < sizeof(changedLines); i++) {
			sprintf(buf, "%d,", changedLines[i]);
			str.append(buf);
		}
		__android_log_print(ANDROID_LOG_INFO, "MagicBox", "DBXDRAW GFX_EndUpdate - [%d,%d,|%s|,%d]",
							startLine, endLine, str.c_str(), sizeof(changedLines));
		//GFX_EndUpdate - [365,367,|365,2,33,0|,4]
		//GFX_EndUpdate - [2,303,|2,13,3,13|,4]
	}*/

	Android_UnlockSurface(startLine, endLine);
}
...
{
   //android rgb16
	return ((red<<8)&0xF800)|((green<<3)&0x07E0)|((blue>>3)&0x1F);
}
...
static void SaveGame(bool pressed)
{
	if (!pressed)
		return;

	SaveState::instance().save("C:\\dbxsave\\");
}

static void LoadGame(bool pressed)
{
	if (!pressed)
		return;

	SaveState::instance().load("C:\\dbxsave\\");
}
...
static void GUI_StartUp(Section * sec) 
{
	...
	MAPPER_AddHandler(&SaveGame, MK_f11, MMOD1, "savestate", "Save State");
	MAPPER_AddHandler(&LoadGame, MK_f11, MMOD1, "loadstate", "Load State");
...
void GFX_Events() 
{
	...
	if (loadf->abort == 2) {
		...
		KEYBOARD_AddKey(KBD_x, false);
		KEYBOARD_AddKey(KBD_i, true);
		KEYBOARD_AddKey(KBD_i, false);
		KEYBOARD_AddKey(KBD_t, true);
		KEYBOARD_AddKey(KBD_t, false);
	...
	while ((!stopPoll) && ((event = Android_PollEvent()) != 0))
	{
//		InputEvent *event = &event1;
		switch (event->eventType) {
			...
			{        
				...
				if ((event->x == -2048) && (event->y == -2048)) {
					...
					if (Mouse_isMicrosoftMouse())
					{
					    //4x(-511) instead 1x(-2048)
						event->x = -511;
						event->y = -511;
				...
				//CatLog::log("c++Moved------SDL_MOUSEMOTION");

				if (loadf->calcSensitivity == 0) {
					Mouse_CursorMoved(event->x - oldx, event->y - oldy, 0, 0, true);
				} else {
					Mouse_CursorMoved((event->x - oldx) * loadf->sensitivity, (event->y - oldy) * loadf->sensitivity, 0, 0, true);
				}
			...
			{      
				...
				{
					//CatLog::log("c++Moved------SDL_MOUSEBUTTONDOWN");
					
					if (!Mouse_isMicrosoftMouse())
					{
						if (loadf->calcSensitivity == 0) {
							Mouse_CursorMoved(0, 0, event->down_x/1000.0f, event->down_y/1000.0f, false);
						} else {
							Mouse_CursorMoved(0, 0,
							event->down_x/1000.0f*loadf->sensitivity,
							event->down_y/1000.0f*loadf->sensitivity, false);
						}
					}
					else
					{
						 //This is fix for absolute mouse under windows. It works, but its crap:(
						 //I added this because I wanted to play Might and Magic 6 : Mandate of Heaven under win98.
						 //But this fix is "blind" and must be done other way. I really don't know where cursor is...and
						 //games mostly manage cursor at their own and they don't tell me about the change...:( 
						 //MM6 works... but playing starcraft or diablo is pain;(
					
						 //CatLog::log("mouse max,y[%d,%d]",Mouse_GetMaxX(),Mouse_GetMaxY());
						 
						 float tmpX = ((event->down_x / 1000) * Mouse_GetMaxX()) - posX;
						 float tmpY = ((event->down_y / 1000) * Mouse_GetMaxY()) - posY;
                  			
						 //CatLog::log("tmpXY 1 [%f,%f]", tmpX, tmpY);
							
						 posX += tmpX;
						 posY += tmpY;								
								
						 if (posX > Mouse_GetMaxX()) posX = Mouse_GetMaxX();
						 if (posX < 0) posX = 0;
				
						 if (posY > Mouse_GetMaxY()) posY = Mouse_GetMaxY();
						 if (posY < 0) posY = 0;								
						
                         tmpX = tmpX / (((tmpX >= -2) && (tmpX <= 2))?1:2);
						 tmpY = tmpY / (((tmpY >= -2) && (tmpY <= 2))?1:2);

						 //ms fix works, but not at 100%, there is little smoothness correction which must be fixed
						 //you can see this desychronisation when dragging mouse
						 /*if ((tmpX < -1) && (tmpX > -3)) tmpX += -0.36;
						 if ((tmpX > 1) && (tmpX < 3)) tmpX += +0.36;
						 
						 if ((tmpY < -1) && (tmpY > -3)) tmpY += -0.36;
						 if ((tmpY > 1) && (tmpY < 3)) tmpY += +0.36;*/
						 
						 //CatLog::log("tmpXY 2 [%f,%f]", tmpX, tmpY);

						 if (loadf->calcSensitivity == 0) {
						 	Mouse_CursorMoved(tmpX, tmpY, 0, 0, true);
						 } else {
						 	Mouse_CursorMoved(tmpX * loadf->sensitivity, tmpY * loadf->sensitivity, 0, 0, true);
						 }
					}
				}

				if (event->keycode >= 0)
					Mouse_ButtonPressed(event->keycode);
			}
				break;
			case SDL_MOUSEBUTTONUP:
			{
				if (event->keycode >= 0)
					Mouse_ButtonReleased(event->keycode);
			}
				break;

			case SDL_DPADAXISMOTION:
			{
				JOYSTICK_Move_X(event->deviceIndex, event->x);
				JOYSTICK_Move_Y(event->deviceIndex, event->y);
			}
			break;

			case SDL_JOYAXISMOTION:
			{
				JOYSTICK_Move_X(event->deviceIndex, event->x/256.0f);
				JOYSTICK_Move_Y(event->deviceIndex, event->y/256.0f);
			}
			break;

			case SDL_JOYBUTTONDOWN:
			{
				JOYSTICK_Button(event->deviceIndex, event->keycode, true);
			}
				break;
			case SDL_JOYBUTTONUP:
			{
				JOYSTICK_Button(event->deviceIndex, event->keycode, false);
			}
				break;
			...
			case SDL_KEYDOWN:
			{
				...
				sdl_event.key.keysym.sym = SDLK_a;	//SDLK_F9;
				sdl_event.key.keysym.mod = KMOD_LCTRL;
				void MAPPER_CheckEvent(SDL_Event * event);
				MAPPER_CheckEvent(&sdl_event);*/
/*
	//__android_log_print(ANDROID_LOG_INFO, "MagicBox", "%d %c", keyCode, letters[keyCode - 29]);
	if (event->keycode != KBD_enter) {
		test2 += letters2[event->aKeycode - 29];
		//__android_log_print(ANDROID_LOG_INFO, "MagicBox", "%d - %c", keyCode, letters[keyCode - 29]);
	} else {
		if (event->eventType == SDL_KEYDOWN) {
			if (test2 != "aabbccddeeffgghhiijjkkllmmnnooppqqrrssttuuvvwwxxyyzz")
			__android_log_print(ANDROID_LOG_INFO, "MagicBox", "c++ DESYNCHRONISATION");
			test2.clear();
		}
	}
*/
				...
				bool shift = ((modifier & KEYBOARD_SHIFT_FLAG) != 0);
				KBD_KEYS kcode=(KBD_KEYS)event->keycode;

				if (down) {
					if (ctrl) KEYBOARD_AddKey(KBD_leftctrl, true);
					if (alt) KEYBOARD_AddKey(KBD_leftalt, true);
					if (shift) KEYBOARD_AddKey(KBD_leftshift, true);

					if (kcode != KBD_NONE)
						KEYBOARD_AddKey(kcode, true);
				}
				else {
					if (kcode != KBD_NONE)
						KEYBOARD_AddKey(kcode, false);

					if (ctrl) KEYBOARD_AddKey(KBD_leftctrl, false);
					if (alt) KEYBOARD_AddKey(KBD_leftalt, false);
...
int dosbox_main(int argc, const char* argv[])
{
	...
	if (!parsed_anyconfigfile && control->ParseConfigFile(config_file.c_str()))
	{
		parsed_anyconfigfile = true;
	}
