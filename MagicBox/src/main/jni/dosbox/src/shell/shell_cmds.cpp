 ...
 */
	...
	try {
		...
		if (command.GetCount() != 2) return;	//throw 0 ;
		command.FindCommand(2,arg);
		if((arg=="/D" ) || (arg=="/d")) return;	//throw 1; //No removal (one day)
  
		command.FindCommand(1,arg);
		if( (arg.size()>1) && arg[1] !=':')  return;	//throw(0);
		temp_str[0]=(char)toupper(args[0]);
		if(Drives[temp_str[0]-'A'] ) return;	//throw 0; //targetdrive in use
		strcat(mountstring,temp_str);
		strcat(mountstring," ");

		command.FindCommand(2,arg);
   		Bit8u drive;char fulldir[DOS_PATHLENGTH];
		if (!DOS_MakeName(const_cast<char*>(arg.c_str()),fulldir,&drive)) return;	//throw 0;
	
		if( ( ldp=dynamic_cast<localDrive*>(Drives[drive])) == 0 ) return;	//throw 0;
