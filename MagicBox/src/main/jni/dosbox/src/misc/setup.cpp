...
Value& Value::copy(Value const& in) throw(WrongType) {
	if (this != &in) { //Selfassigment!
		if(type != V_NONE && type != in.type) return *this;	//throw WrongType();
		destroy();
		plaincopy(in);
	}
	return *this;
...
Value::operator bool () const throw(WrongType) {
	if(type != V_BOOL) return 0;	//throw WrongType();
	return _bool;
}

Value::operator Hex () const throw(WrongType) {
	if(type != V_HEX) return 0;	//throw WrongType();
	return _hex;
}

Value::operator int () const throw(WrongType) {
	if(type != V_INT) return 0;	//throw WrongType();
	return _int;
}

Value::operator double () const throw(WrongType) {
	if(type != V_DOUBLE) return 0;	//throw WrongType();
	return _double;
}

Value::operator char const* () const throw(WrongType) {
	if(type != V_STRING) return 0;	//throw WrongType();
...
bool Value::SetValue(string const& in,Etype _type) throw(WrongType) {
	 ...
	 */
	if(_type == V_CURRENT && type == V_NONE) return false;	//throw WrongType();
	if(_type != V_CURRENT) { 
		if(type != V_NONE && type != _type) return false;	//throw WrongType();
		type = _type;
	}
	bool retval = true;
	switch(type){
...
bool Prop_int::SetVal(Value const& in, bool forced, bool warn) {
	...
	} else {//Handle ranges if specified
		int mi = min;
		int ma = max;
